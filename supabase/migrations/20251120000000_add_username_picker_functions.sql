...
--
-- VALIDATION LOGIC:
-- 1. Combine random adjective + noun (PascalCase)
-- 2. Check against reserved names list
-- 3. Check against existing usernames in database
-- 4. Check for duplicates within the batch of 10
-- 5. Repeat until 10 unique, available usernames found
--
-- EDGE CASES:
-- - If username < 3 chars: append random 3-digit number (e.g., "TinyBee427")
-- - If username > 35 chars: truncate to 35 chars
-- - Max 100 attempts to prevent infinite loops
--
-- ERROR HANDLING:
-- - Returns partial results if can't find 10 usernames in 100 attempts
-- - Never returns empty array (always at least tries)
--
...
DECLARE
  ...
  generated_usernames text[] := ARRAY[]::text[];  -- Array to collect 10 usernames
  username_lower text;                            -- Lowercase version for storage
  username_display text;                          -- PascalCase version for display
  attempt_count integer := 0;                     -- Prevent infinite loops
  max_attempts integer := 100;                    -- Safety limit
  adjective text;
  noun text;
  random_suffix text;
  existing_usernames text[];                      -- Batch check optimization
BEGIN
  -- Generate 10 unique, available usernames
  WHILE COALESCE(array_length(generated_usernames, 1), 0) < 10 AND attempt_count < max_attempts LOOP
    ...
    -- Validation 1: Not in reserved names list
    IF username_lower = ANY(reserved_names) THEN
      CONTINUE;
    END IF;

    -- Validation 2: Not a duplicate within current batch
    IF username_display = ANY(generated_usernames) THEN
      CONTINUE;
    END IF;

    -- Validation 3: Not already taken in database (batch check for efficiency)
    -- We check all generated usernames in a single query at the end
    -- For now, just add to the array
    generated_usernames := array_append(generated_usernames, username_display);
  END LOOP;

  -- Batch validation: Check all generated usernames against database
  -- Single query instead of 10 separate queries (10x performance improvement)
  SELECT array_agg(lower(username))
  INTO existing_usernames
  FROM public.profiles
  WHERE lower(username) = ANY(
    SELECT lower(unnest(generated_usernames))
  );

  -- Remove any usernames that are already taken
  IF existing_usernames IS NOT NULL THEN
    generated_usernames := ARRAY(
      SELECT unnest(generated_usernames)
      EXCEPT
      SELECT u FROM unnest(existing_usernames) AS u
    );
  END IF;

  -- Return as jsonb with "suggestions" key
  -- Frontend expects: {"suggestions": ["BrightPanda", "SwiftFalcon", ...]}
  RETURN jsonb_build_object('suggestions', generated_usernames);
END;
$function$;
...
BEGIN
  ...
  -- Validation 1: Format check (alphanumeric only)
  -- Regex: ^[a-z0-9]+$ means only lowercase letters and numbers
  IF username_normalized !~ '^[a-z0-9]+$' THEN
