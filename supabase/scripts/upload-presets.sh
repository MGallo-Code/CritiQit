...
# Script to upload preset avatar images to the avatar-presets bucket
...
echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
echo "Uploading preset avatars to the avatar-presets bucket..."
echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
...
for filepath in ./seed/avatar-presets/*.png; do
    ...
    HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
        -X GET "${API_EXTERNAL_URL}/storage/v1/object/public/avatar-presets/${filename}" \
    ...
    if [ "${HTTP_STATUS}" -eq 404 ] || [ "${HTTP_STATUS}" -eq 400 ]; then
        ...
        curl -s -o /dev/null -X POST "${API_EXTERNAL_URL}/storage/v1/object/avatar-presets/${filename}" \
