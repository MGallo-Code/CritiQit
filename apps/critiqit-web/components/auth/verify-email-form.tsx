...
export interface VerifyEmailFormProps extends CardProps {
  initialEmail?: string;
  redirectTo?: string;
}
...
export function VerifyEmailForm({
  ...
  // reset turnstile token when form is submitted
  useEffect(() => {
  ...
  const feedback = useMemo(() => {
    ...
    if (resendState.status === "success") {
      return {
        tone: "success",
        message: "A new verification email has been sent if the address exists.",
      } as const;
    }
    return null;
  ...
  return (
    <Card {...props} className={cn("w-full", className)}>
      <CardHeader>
        ...
        <CardDescription>
          Check your email for a six-digit code to finish creating your account.
        </CardDescription>
      </CardHeader>
